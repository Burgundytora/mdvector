...
int main() {
  ...
  // 情况1: 最后一维完整切片
  subspan<double, 2> sub_contig1 = mat.view(1,     // 选择第1行
                                            all()  // 所有列
  );

  auto sub_contig_test = mat.view(2,     // 选择第1行
                                  all()  // 所有列
  );
  ...
  // 情况1: 最后一维完整切片
  auto sub_contig11 = mat.view(1,            // 选择第1行
                               slice(1, -1)  // 第一列之后
  );
  ...
  // 情况1: 最后一维完整切片
  auto sub_contig111 = mat.view(-1,    // 选择第1行
                                all()  // 第一列之后
  );
  ...
  // 情况2: 单行选择
  auto sub_contig2 = mat.view(0,           // 只选择第1行
                              slice(0, 1)  // 选择第1-2列
  );
  ...
  std::cout << "\n\n=== 测试非法子视图 ===" << std::endl;
  try {
    auto invalid_sub = mat.view(slice(0, 2, false),  // 多行
                                slice(0, 2, false)   // 多列
    );
  ...
  // 测试3: 使用语法糖创建子视图
  std::cout << "\n=== 测试3: 使用语法糖 ===" << std::endl;
  auto sub2 = mat.view(slice(0, 1),  // 第0-1行
                       all()         // 所有列
  );
  ...
  // // 测试5: 尝试创建非法子视图
  try {
    auto invalid_sub = mat.view(slice(2, 4),  // 超出范围
                                slice(0, 3));
  ...
  auto tensor_sub = tensor.view(1,     // 第一维 第二个
                                2,     // 第二维 第三个
                                all()  // 所有第三维
  );
