...
class Expr {
 public:
  ...
  FORCE_INLINE auto eval_simd(size_t i) const { return static_cast<const Derived&>(*this).eval_simd(i); }

  template <typename Dest>
  FORCE_INLINE void eval_to(Dest* dest) const {
    const size_t n = size();
...
class AddExpr : public Expr<AddExpr<L, R>> {
 ...
 public:
  ...
  template <typename T>
  FORCE_INLINE typename SimdConfig<T>::simd_type eval_simd(size_t i) const {
  ...
  template <typename T>
  FORCE_INLINE typename SimdConfig<T>::simd_type eval_simd_mask(size_t i) const {
