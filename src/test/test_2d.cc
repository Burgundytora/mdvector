...
void test_openblas() {
  ...
  TimerRecorder a("blas 1d");

  size_t k = 0;
  while (k++ < loop) {
    if constexpr (do_add) {
      blas_add(data1_, data2_, data3_, total_element);
    }

    if constexpr (do_sub) {
      blas_sub(data1_, data2_, data3_, total_element);
    }

    if constexpr (do_mul) {
      blas_mul(data1_, data2_, data3_, total_element);
    }

    if constexpr (do_div) {
      blas_div(data1_, data2_, data3_, total_element);
    }
  }
