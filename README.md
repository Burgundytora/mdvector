...
**4.md::span<type, Rank>**: 跨步连续视图，对md::vector进行切片创建
...
- **任意维度支持**：通过自定义实现（C++17下）与std::mdpan（c++23下）实现多维索引功能
...
- **内存连续视图**：支持自定义指针偏移实现内存连续切片（`span`）， 切片同样支持高性能表达式模板/数学函数计算操作，内存连续视图的计算、取值、赋值与
